...
/*************  ✨ Windsurf Command 🌟  *************/
    /**
     * Index page for admin prestasi management
     *
     * @return \Illuminate\Http\Response
     */
    public function index()
    {
        // Get all prestasi with user relation
        $prestasiQuery = Prestasi::with('user');

        // Filter by status (using the correct database field)
        
        // Filter berdasarkan status (menggunakan field database yang benar)
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $validated = $request->validate([
                ...
                'nim' => 'required|string|max:20',
                'semester' => 'required|integer|min:1|max:8',
                'status' => 'nullable|in:Menunggu Validasi,Tervalidasi,Ditolak'
            ]);

            // Handle file upload
            if ($request->hasFile('bukti')) {
                if ($prestasi->bukti_prestasi) {
                    \Illuminate\Support\Facades\Storage::disk('public')->delete($prestasi->bukti_prestasi);
                }
                $file = $request->file('bukti');
                $fileName = time() . '_' . $file->getClientOriginalName();
                $filePath = $file->storeAs('bukti_prestasi', $fileName, 'public');
            ...
            // Map display status to database values if provided
            if (isset($validated['status'])) {
                ...
                $validated['status_validasi'] = $statusMapping[$validated['status']];
    ...
    public function bulkAction(Request $request)
    {
        try {
            $request->validate([
                'ids' => 'required|array',
                'action' => 'required|in:delete,validate,reject'
            ]);

            $ids = $request->ids;

            if ($request->action === 'delete') {
                Prestasi::whereIn('id_prestasi', $ids)->delete();
                return response()->json(['success' => true, 'message' => 'Data berhasil dihapus']);
            } elseif ($request->action === 'validate') {
                Prestasi::whereIn('id_prestasi', $ids)->update(['status_validasi' => 'disetujui']);
                return response()->json(['success' => true, 'message' => 'Data berhasil divalidasi']);
            } elseif ($request->action === 'reject') {
                Prestasi::whereIn('id_prestasi', $ids)->update(['status_validasi' => 'ditolak']);
